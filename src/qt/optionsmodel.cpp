...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeRupxAmount") || reset)
        gArgs.SoftSetArg("-anonymizerupxamount", settings.value("nAnonymizeRupxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rupx.cpp during shutdown to also remove the window positions
