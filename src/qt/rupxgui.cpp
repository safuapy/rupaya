...
const QString RupxGUI::DEFAULT_WALLET = "~Default";

RupxGUI::RupxGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void RupxGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &RupxGUI::toggleHidden);
...
void RupxGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &RupxGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &RupxGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &RupxGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &RupxGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &RupxGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &RupxGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &RupxGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &RupxGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &RupxGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &RupxGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &RupxGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &RupxGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &RupxGUI::execDialog);
}


void RupxGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void RupxGUI::handleRestart(QStringList args)
{
...
void RupxGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &RupxGUI::message);
...
void RupxGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &RupxGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &RupxGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void RupxGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void RupxGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void RupxGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &RupxGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &RupxGUI::show);
                e->ignore();
            }
        }
    }
...
void RupxGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void RupxGUI::messageInfo(const QString& text)
{
...
void RupxGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool RupxGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void RupxGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void RupxGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RupxGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void RupxGUI::goToSend()
{
    showTop(sendWidget);
}

void RupxGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void RupxGUI::goToMasterNodes()
{
    masterNodesWidget->resetCoinControl();
    showTop(masterNodesWidget);
}

void RupxGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void RupxGUI::goToGovernance()
{
    showTop(governancewidget);
}

void RupxGUI::goToSettings(){
    showTop(settingsWidget);
}

void RupxGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void RupxGUI::goToReceive()
{
    showTop(receiveWidget);
}

void RupxGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void RupxGUI::showTop(QWidget* view)
{
...
void RupxGUI::changeTheme(bool isLightTheme)
{
...
void RupxGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool RupxGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void RupxGUI::showHide(bool show)
{
...
int RupxGUI::getNavWidth()
{
    return this->navMenu->width();
}

void RupxGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void RupxGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void RupxGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool RupxGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &RupxGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &RupxGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &RupxGUI::message);
    connect(topBar, &TopBar::message, this, &RupxGUI::message);
    connect(sendWidget, &SendWidget::message,this, &RupxGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &RupxGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &RupxGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &RupxGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &RupxGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &RupxGUI::incomingTransaction);

    return true;
}

bool RupxGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void RupxGUI::removeAllWallets()
{
    // Single wallet supported.
}

void RupxGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(RupxGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void RupxGUI::subscribeToCoreSignals()
{
...
void RupxGUI::unsubscribeFromCoreSignals()
{
