...
SettingsFaqWidget::SettingsFaqWidget(RupxGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("Rupaya is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. Rupaya incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("Rupaya utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of Rupaya to participate in earning block rewards "
    ...
    QString unspendablePIVContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received Rupaya requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your Rupaya wallet also needs to be completely synchronized "
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of Rupaya with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("rupx-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.Rupaya.org'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in Rupaya Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the Rupaya network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    if (parent)
        connect(parent, &RupxGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
