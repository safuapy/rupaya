...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " rupx-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  rupx-tx [options] <hex-tx> [commands]  Update hex-encoded rupaya transaction\n" +
                               "or:     rupx-tx [options] -create [commands]   Create hex-encoded rupaya transaction\n" +
                               "\n";
